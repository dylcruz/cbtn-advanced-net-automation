...
def replace_config(task):
    template_results = task.run(task=template_file, template=f'{task.host}.j2', path='./templates')
...
os.system(CURRENT)
command = subprocess.run(['pyats', 'diff', 'golden-config', 'current-config',
                          '--output', 'configs-diff'], stdout=subprocess.PIPE)
str_command = str(command)
if 'Diff can be found' in str_command:
    os.system(CLEAR)
    print('ALERT: POTENTIAL PROBLEM DETECTED')
    answer = input('Would you like to reverse the current configs back to their'
                   'golden state? <y/n>: ')
    if answer.lower() == 'y':
        def main():
            os.system(CLEAN)
            os.system(CLEAR)
            nr.run(load_vars)
            replace_results = nr.run(replace_config)
